...
8. During process update `dist/config/decision-log.md` with your plan and: important decisions, constraints, bug fixes, and what files have been updated with new/delete row count.  Do not store chat transcripts, code dumps, or temporary notes.
9. Review the persistent decision log at `dist/config/decision-log.md` before acting.
...
- For multi-step tasks, create a TODO checklist in `dist/config/decision-log.md` under the `## TODO` section.
...
When the user picks a mockup from `dist/config/` to implement:
...
7. If the startup skill asked you to create mockups, those mockup files must actually be written to `dist/config/` before you say they are available.
8. Mockups in `dist/config/` are standalone one-page HTML prototypes. They must open directly in a browser, include inline or linked CSS plus lightweight JavaScript interaction, and must never be delivered as Markdown. Include a navigation button that links to the next mockup.
...
- Local images get corrupted, inline the png/jpg/gif as base64 data URIs so images do not get corrupted. External image URLs are okay to use.
...
- Shipped app implementation files live in the `dist/` directory. Schema and startup workflow artifacts may also live in `dist/config/`:
  - `dist/config/decision-log.md`
  - `dist/config/mockup-1.html` through `dist/config/mockup-5.html` (or similar mockup names)
- If you need to write startup artifacts under `dist/config/`, ensure that directory exists first.
